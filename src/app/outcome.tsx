...
const videos = {
  "1": require("../../assets/videos/karar-1.mp4"),
  "2": require("../../assets/videos/karar-2.mp4"),
} as const;

/** Figma: Karar 1 (1635:893) and Karar 2 (1639:1263). Video fill scaleMode FILL, 812×375. */
export default function OutcomeScreen() {
  const { scene } = useLocalSearchParams<{ scene?: string }>();
  const key = scene === "2" ? "2" : "1";

  return (
    <PlayFrame
      background={<SceneVideo key={key} source={videos[key]} style={StyleSheet.absoluteFill} />}
    >
      <Pressable
        accessibilityRole="button"
        accessibilityLabel={key === "1" ? "Next outcome" : "Back"}
        onPress={() => (key === "1" ? router.push("/outcome?scene=2") : router.back())}
        style={styles.hit}
      />
